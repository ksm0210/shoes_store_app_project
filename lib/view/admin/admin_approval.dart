...
class _AdminApprovalScreenState extends State<AdminApprovalScreen> {
  ...
  void initState() {
    super.initState();
    // 중복 제거된 제품명 목록 생성
    _availableProducts = controller.allInventory
        .map((item) => item.productName)
        .toSet()
        .toList();
    _selectedProduct = _availableProducts.isNotEmpty
        ? _availableProducts.first
        : null;
  }
    ...
    return Form(
      ...
      child: ListView(
        ...
        children: [
          ...
          // 1. 재고 부족 알림 목록
          if (controller.lowStockItems.isNotEmpty) _buildLowStockWarning(),
          ...
          TextFormField(
            ...
            validator: (value) =>
                value == null || value.isEmpty ? '사유를 입력해주세요.' : null,
          ),
  ...
  Widget _buildApprovalView() {
    ...
    final ordersToApprove = controller.purchaseOrders.where((order) {
      if (isExecutive) {
        ...
        return order.status.value == ApprovalStatus.teamApproved ||
            order.status.value == ApprovalStatus.pending;
      } else {
    ...
    final approvedOrders = controller.purchaseOrders
        .where(
          (order) => order.status.value == ApprovalStatus.executiveApproved,
        )
        .toList();
      ...
      () => ListView(
        ...
        children: [
          ...
          if (ordersToApprove.isEmpty)
            ...
            ...ordersToApprove
                .map((order) => _buildOrderApprovalTile(order, isExecutive))
                .toList(),
  ...
  Widget _buildOrderApprovalTile(PurchaseOrder order, bool isExecutive) {
    final isFinalApproval =
        order.status.value == ApprovalStatus.teamApproved && isExecutive;
    ...
    return Card(
      ...
      child: Padding(
        ...
        child: Column(
          ...
          children: [
            Text(
              "품의 ID: ${order.orderId} (${order.statusText})",
            ...
            Text(
              "요청자: ${order.requesterName} (${DateFormat('MM/dd').format(order.dateRequested)})",
            ...
            Row(
              ...
              children: [
                TextButton(
                  onPressed: () =>
                      controller.processApproval(order.orderId, false), // 반려
                  child: const Text("반려", style: TextStyle(color: Colors.red)),
                ),
                const SizedBox(width: 10),
                ElevatedButton(
                  onPressed: () =>
                      controller.processApproval(order.orderId, true), // 승인
                  style: ElevatedButton.styleFrom(
                    backgroundColor: isFinalApproval
                        ? Colors.black
                        : Colors.green,
                  ),
                  child: Text(
  ...
  Widget _buildApprovedOrderTile(PurchaseOrder order) {
    return ListTile(
      ...
      subtitle: Text(
        "요청일: ${DateFormat('MM/dd').format(order.dateRequested)} | 최종 승인 완료",
      ),
