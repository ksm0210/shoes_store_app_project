...
class AdminInventoryScreen extends StatelessWidget {
  ...
  Widget _buildStoreInventoryView(AdminController controller) {
    ...
    const String targetStoreId = 'S001';

    return Obx(() {
      final storeInventory = controller.allInventory
          .where((item) => item.storeId == targetStoreId)
          .toList();
      return ListView(
  ...
  Widget _buildLowStockTile(InventoryItem item) {
    return Card(
      ...
      child: ListTile(
        ...
        subtitle: Text(
          "${item.storeName} | 재고 ${item.stockCount} / ${item.maxStock}",
        ),
        trailing: Text(
    ...
    return Container(
      ...
      child: SingleChildScrollView(
        ...
        child: DataTable(
          ...
          columns: const [
            DataColumn(
              label: Text('제품명', style: TextStyle(fontWeight: FontWeight.bold)),
            ),
            DataColumn(
          ...
          rows: salesData.map((data) {
            final revenueFormatted = currencyFormatter.format(
              data['totalRevenue'],
            );
              ...
              cells: [
                ...
                DataCell(
                  Text(
                    '₩$revenueFormatted',
                    style: TextStyle(
                      color: data['totalRevenue'] > 5000000
