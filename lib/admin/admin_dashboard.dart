...
class AdminDashboardScreen extends StatelessWidget {
  ...
  // 관리자 역할에 따른 메뉴 위젯 생성
  Widget _buildRoleSpecificMenu(BuildContext context, AdminRole role, AdminController controller) {
    switch (role) {
  ...
  // -----------------------------------------------------------
  Widget _buildStoreManagerMenu(BuildContext context, AdminController controller) {
    return Column(
      ...
      children: [
        _buildDashboardCard(
          ...
          onTap: () {
            ...
            Navigator.push(context, MaterialPageRoute(builder: (context) => const AdminInventoryScreen(role: AdminRole.storeManager)));
          },
        ),
        _buildDashboardCard(
          title: "고객 반품 요청 확인",
        ...
        const Text("오늘의 주요 활동", style: TextStyle(fontSize: 18, fontWeight: FontWeight.bold)),
        const SizedBox(height: 10),
        ListTile(
          leading: const Icon(Icons.warning, color: Colors.orange),
          title: const Text("재고 부족 알림"),
          subtitle: Text("재고가 60% 미만인 상품 ${controller.lowStockItems.length}개가 있습니다."),
        )
      ],
    );
  }
  ...
  // -----------------------------------------------------------
  Widget _buildHeadOfficeMenu(BuildContext context, AdminController controller) {
    return Column(
      crossAxisAlignment: CrossAxisAlignment.start,
      children: [
        _buildDashboardCard(
          ...
          onTap: () {
            ...
            Get.snackbar("발송 관리", "금일 구매 신청 5건, 강남점으로 3건 발송 예정", snackPosition: SnackPosition.BOTTOM);
          },
        ),
        _buildDashboardCard(
          ...
          onTap: () {
             Navigator.push(context, MaterialPageRoute(builder: (context) => const AdminInventoryScreen(role: AdminRole.headOffice)));
          },
        ),
        _buildDashboardCard(
          title: "신발 발주 품의 시스템",
  ...
  Widget _buildExecutiveMenu(BuildContext context, AdminController controller) {
    final pendingOrdersCount = controller.purchaseOrders.where((o) => o.status.value == ApprovalStatus.teamApproved).length;
    ...
    return Column(
      ...
      children: [
        _buildDashboardCard(
          ...
          onTap: () {
            Navigator.push(context, MaterialPageRoute(builder: (context) => const AdminApprovalScreen(isApprovalMode: true)));
          },
        ),
        _buildDashboardCard(
          title: "전사 판매 현황 보고서",
          ...
          onTap: () {
             Navigator.push(context, MaterialPageRoute(builder: (context) => const AdminInventoryScreen(role: AdminRole.executive)));
          },
        ),
      ],
    );
      ...
      body: ListView.builder(
        ...
        itemBuilder: (context, index) {
          ...
          return ListTile(
            ...
            onTap: () {
               Get.snackbar("처리", "${req.requestId} 반품을 처리합니다.", snackPosition: SnackPosition.BOTTOM);
            },
          );
        },
      ),
      ...
      return Scaffold(
        ...
        body: SingleChildScrollView(
          ...
          child: Column(
            ...
            children: [
              Text(
                "환영합니다, ${currentRole == AdminRole.executive ? '이사' : currentRole == AdminRole.storeManager ? '대리점장' : '사원'}님.",
